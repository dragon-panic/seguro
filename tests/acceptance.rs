...
/// Persona injection: system_prompt is written to workspace/.claude/CLAUDE.md
/// and persona env vars are available inside the guest.
#[tokio::test]
#[ignore = "requires KVM and built base image"]
async fn persona_injection_system_prompt_and_env() {
    use seguro::api::{OutputMode, Sandbox, SandboxConfig};
    ...
    // Write a persona TOML
    let persona_path = tmp.path().join("persona.toml");
    std::fs::write(
        &persona_path,
        r#"
...
"#,
    ...
    let result = sandbox
        .exec(&["cat".into(), ".claude/CLAUDE.md".into()])
        .await
        .expect("exec failed");
    let stdout_bytes = result.stdout.unwrap_or_default();
    let stdout = String::from_utf8_lossy(&stdout_bytes);
    assert!(
        stdout.contains("security auditor"),
        "expected persona prompt in CLAUDE.md, got: {:?}",
        stdout
    );

    // Verify env vars
    let result = sandbox
        .exec(&["bash".into(), "-c".into(), "echo $AGENT_MODE $SEGURO_ROLE".into()])
        .await
        .expect("exec env failed");
    let env_bytes = result.stdout.unwrap_or_default();
    let stdout = String::from_utf8_lossy(&env_bytes);
    assert!(stdout.contains("audit"), "AGENT_MODE not set, got: {:?}", stdout);
    assert!(stdout.contains("auditor"), "SEGURO_ROLE not set, got: {:?}", stdout);
